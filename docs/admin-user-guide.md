...
| 메뉴 | 경로 | 기능 |
| --- | --- | --- |
| 사용자 관리 | `/admin/users` | 사용자 계정, 조직, 직위, 역할, 활성 상태 조회 |
| 조직 관리 | `/admin/organizations` | 조직명, 상위 조직, 레벨, 정렬, 활성 상태 조회 |
| 직위 관리 | `/admin/positions` | 직위명, 직위 순서, 정렬, 활성 상태 조회 |
| 결재선 관리 | `/admin/approval-lines` | 결재 유형별 결재 단계와 결재선 상태 조회 |
| 예외 결재자 | `/admin/approval-org-exceptions` | 조직별 예외 결재자 조회 |
| 전체 신청서 | `/admin/applications` | 전체 신청서 조회, 상태 필터, 관리자 예외 결재 |
| 알림 로그 | `/admin/notifications` | 알림 유형, 채널, 발송 상태, 읽음 상태 조회 |

현재 관리자 기준정보 화면은 UI상 조회 중심이다. 기준정보 생성은 Backend 관리자 API를 사용하거나, 운영 정책에 맞는 별도 관리 UI를 추가해야 한다.
